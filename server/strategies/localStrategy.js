...
function localStrategy(passport) {
    passport.use(new Strategy({
        ...
        try {
            const user = await DB.one('SELECT * FROM movie_users WHERE username = $1', [username]);
    ...
    passport.deserializeUser(async (id,done) => {
        try {
            const user = await DB.any('SELECT id, username, email, user_role FROM movie_users WHERE id = $1',[id]);
